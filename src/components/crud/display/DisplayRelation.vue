<template>
  <div class="awesome-display-relation" v-bind="$props">
    <template v-for="value in _values">
      <div :key="value">
        <span v-if="value" class="badge badge-info" :style="innerStyles">
          {{ getLabel(value) }}
        </span>
        <router-link v-if="value && $props.onClickUrl" :to="$props.onClickUrl + '/' + value" class="external-link">
          &nbsp; <i class="fa fa-external-link text-info"></i>
        </router-link>

        <span
          v-if="value && $props.onClickUrl"
          :to="$props.onClickUrl + '/' + value"
          class="copy-link"
          @click="copy(value)"
        >
...
export default {
  ...
  methods: {
    ...
    getStoreLabel(value) {
      if (value && this._displayLabelCache[value]) {
    ...
    getApiLabel(value) {
      ...
      if (this._displayLabelCache[url]) {
...
.awesome-display-relation .external-link {
  visibility: hidden;
  opacity: 0;
  transition: all 50ms linear 500ms;
  position: absolute;
  padding-right: 50px;
  cursor: pointer;
}
